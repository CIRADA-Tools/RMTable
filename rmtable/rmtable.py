...
class RMTable:
    ...
    def write_tsv(self, filename):
        ...
        for i in range(len(self.columns)):
            if ("U" not in self.dtype[i]) and ("S" not in self.dtype[i]):
                continue
            w = np.where(
                np.char.find(self.table[self.columns[i]].data.astype(str), "\t") != -1
            )[0]
            if w.size > 0:
                print('Illegal tabs detected! Replaced with "@@"')
                self.table[self.columns[i]][w] = np.char.replace(
                    self.table[self.columns[i]][w].data.astype(str), "\t", "@@"
                )
            # Similarly, check for newlines, and replace with double space ('  ')
            w = np.where(
                np.char.find(self.table[self.columns[i]].data.astype(str), "\n") != -1
            )[0]
            if w.size > 0:
    ...
    def read_tsv(self, filename):
        ...
        table = np.genfromtxt(
            filename, dtype=None, names=True, delimiter="\t", encoding=None
        )
    ...
    def read_VOTable(self, filename):
        ...
        self.units = [
            y.unit.to_string("vounit") if y.unit is not None else ""
            for y in table.fields
        ]
    ...
    def input_numpy(
        self, array, verbose=False, verify=True, keep_cols=[], coordinate_system="icrs"
    ):
        ...
        if ("ra" in matching_columns) and ("l" in matching_columns):
            ...
            # calculate Galactic:
            long, lat = calculate_missing_coordinates_column(
        ...
        for col in missing_columns:
            newtable[self.columns.index(col)] = np.repeat(
                self.standard_blanks[self.columns.index(col)], Nrows
            )
            print(col)
        print()

        for i in range(len(newtable)):
            newtable[i] = at.Column(
                data=newtable[i], name=self.columns[i], dtype=self.standard_dtypes[i]
            )

        # Turn everything into a RMtable, and append:
        newrows = at.Table(
            data=newtable, names=self.columns, dtype=self.standard_dtypes
        )
...
def input_numpy(
    array, verbose=False, verify=False, keep_cols=[], coordinate_system="icrs"
):
    """Converts an input numpy array into an RM table object.
    Requires that array has named columns matching standard column names.
    Will automatically fill in missing columns with default values.
